...
$(document).ready(function(){
  ...
  $.ajax({
    ...
    $.each(bill_arr.data, function(index,bill){
      ...
      rows +="<td class='option'><button class='btn-primary update-bill' value='Edit' data-toggle='modal' data-target='#update-bill-Modal'>EDIT</button><button class='btn-warning bill-detail' value='DETAIL' data-toggle='modal' data-target='#bill-detail-Modal'>DETAIL</button></td>";
      rows +="</tr>";
    })
...
//     });
    ...
    $('#add-billitem-btn').click(function(){
        var proname = $('#bill-productname').val();
        var procode = $('#bill-productcode').val();
      ...
      rows +="<td class='bill-itemcode' >"+procode+"</td>";
